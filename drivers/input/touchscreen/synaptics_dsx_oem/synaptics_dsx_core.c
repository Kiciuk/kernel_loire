...
*/
bool is_tp_insert = false;

bool tp_sleep_mode = false;
...
struct synaptics_rmi4_data *rmi4_data_truly;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		{
			if((current_status[button] == 1 && status == 1))
			{
				input_report_key(rmi4_data->input_dev,
					f1a->button_map[button],
					!status);
				input_sync(rmi4_data->input_dev);
				printk("synaptics release virtual key(touch up)!\n");
			}
			else
			{
				continue;
...
int synaptics_rmi4_int_enable(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval = 0;
	unsigned char ii;
		...
		bool enable, bool attn_only)
{
	...
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	SYNAP_INFO("Enter synaptics_rmi4_irq_enable status:%s",enable ? "enable" : "disable");
	...
	if (enable) {
		...
		{
			SYNAP_INFO("synaptics already in irq enable");
			return retval;
		}
...
flash_prog_mode:
	...
	retval = secure_memcpy(rmi->product_id_string,
			...
			PRODUCT_ID_SIZE);
	SYNAP_INFO("product_id_string:%s",rmi->product_id_string);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		...
		retval = gpio_request(gpio, "dsx_gpio");
		if (retval) {
		...
		bool rebuild)
{
	int retval;
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	SYNAP_INFO("Enter %s rebuild:%d",__func__,rebuild);
...
#endif
void synaptics_rmi4_sleep_enable(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval;
	unsigned char device_ctrl;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			&device_ctrl,
			sizeof(device_ctrl));
	printk("synaptics device_ctrl after write:0x%x\n",device_ctrl);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s %d: Failed to enter sleep mode\n",
				__func__,__LINE__);
		return;
	}
...
static void synaptics_ts_pm_worker(struct work_struct *work)
{
	int ret = -ENOMEM;
	struct synaptics_rmi4_data *rmi4_data  = container_of(work, struct synaptics_rmi4_data,
	synaptics_pm_work.work);

	if (!rmi4_data) {
		ret = -ENOMEM;
		printk(KERN_ERR"synaptics_ts is null!\n");
		return ;
	}

	printk("%s():lcd status \%s\n",__func__,(rmi4_data->pm_status ==
	       FB_BLANK_UNBLANK)?"FB_BLANK_UNBLANK":"FB_BLANK_POWERDOWN");

	if (rmi4_data->pm_status == FB_BLANK_UNBLANK){
		synaptics_rmi4_resume(&rmi4_data->pdev->dev);
		rmi4_data->fb_ready = true;}
	else if (rmi4_data->pm_status == FB_BLANK_POWERDOWN){
...
static int synaptics_pinctrl_init(struct synaptics_rmi4_data *rmi4_data)
 {
	 ...
	 rmi4_data->client = client;
	 SYNAP_INFO("%s, enter!!!", __func__);
	 rmi4_data->synaptics_pinctrl = devm_pinctrl_get(&(rmi4_data->client->dev));
	 if (IS_ERR_OR_NULL(rmi4_data->synaptics_pinctrl)) {
		 dev_err(&rmi4_data->client->dev, "Synaptics Failed to get pinctrl\n");
		 retval=PTR_ERR(rmi4_data->synaptics_pinctrl);
     rmi4_data->synaptics_pinctrl = NULL;
     return retval;
	 }

	 rmi4_data->gpio_state_active =
		 pinctrl_lookup_state(rmi4_data->synaptics_pinctrl, "pmx_ts_active");
	 if (IS_ERR_OR_NULL(rmi4_data->gpio_state_active)) {
		 dev_err(&rmi4_data->client->dev, "Synaptics Failed to look up default state\n");
	 ...
	 rmi4_data->gpio_state_suspend =
		 pinctrl_lookup_state(rmi4_data->synaptics_pinctrl, "pmx_ts_suspend");
	 if (IS_ERR_OR_NULL(rmi4_data->gpio_state_suspend)) {
		 dev_err(&rmi4_data->client->dev, "Synaptics Failed to look up sleep state\n");
...
static void synaptics_power_on_gpio_set(struct synaptics_rmi4_data *rmi4_data)
{
	...
	SYNAP_INFO("Enter %s",__func__);
	gpio_direction_output(rmi4_data->hw_if->board_data->reset_gpio, 0);//pull down
	msleep(rmi4_data->hw_if->board_data->reset_active_ms);
	gpio_direction_output(rmi4_data->hw_if->board_data->reset_gpio, 1);//pull up
	msleep(rmi4_data->hw_if->board_data->reset_delay_ms);
	//gpio_direction_input(rmi4_data->hw_if->board_data->irq_gpio);
	//rc = pinctrl_select_state(rmi4_data->synaptics_pinctrl, rmi4_data->gpio_state_active);
	//if (rc < 0)
	//{
	//    printk("synaptics_pinctrl_select_state err!!!\n");
	//    return;
	//}
	//msleep(5);
	return;
...
{
	int rc;
	SYNAP_INFO("Enter %s:gpio power off",__func__);
	rc = pinctrl_select_state(rmi4_data->synaptics_pinctrl, rmi4_data->gpio_state_suspend);
	if (rc < 0)
	{
	    printk("synaptics_pinctrl_select_state err!!!\n");
	    return;
	...
	//gpio_direction_input(rmi4_data->hw_if->board_data->irq_gpio);
	//gpio_direction_input(rmi4_data->hw_if->board_data->reset_gpio);
		...
		unsigned int address)
{
	...
	unsigned char page;
	//struct i2c_client *client = to_i2c_client(rmi4_data->pdev->dev.parent);
	struct i2c_client *i2c = rmi4_data->client;
	printk("%s enter!!!\n",__func__);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	//struct i2c_client *client = to_i2c_client(rmi4_data->pdev->dev.parent);
	SYNAP_INFO("synaptics_rmi4_i2c_read_nodsm enter!");
	...
	read_msg[0].addr = rmi4_data->client->addr;
	read_msg[0].flags = 0;
	read_msg[0].len = 1;
	read_msg[0].buf = &buf;
	...
	for ( ii = 1; ii <= total; ii++) {
		read_msg[ii].addr = rmi4_data->client->addr;
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		if (i2c_transfer(rmi4_data->client->adapter, read_msg, (total + 1)) == (total + 1)) {
			retval = length;
			break;
		}
		printk("%s: I2C retry %d\n",
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		printk("%s: I2C read over retry limit\n",
				__func__);
		retval = -EIO;
	}
...
static int i2c_communicate_check(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	int i;
	unsigned short pdt_entry_addr = PDT_START;
	...
	for (i = 0; i < I2C_CHECK_RETRY_TIMES; i++)
	{
		...
		if (retval < 0)
		{
			printk("synaptics %s: Failed to read register map, i = %d, retval = %d\n", __func__, i, retval);
			if(!pull_down_flag)
			{
			 gpio_direction_output(bdata->reset_gpio, 1);//pull up
			 msleep(5);
			 gpio_direction_output(bdata->reset_gpio, 0);//pull down
			 msleep(bdata->reset_active_ms);
			 gpio_direction_output(bdata->reset_gpio, 1);//pull up
			 msleep(bdata->reset_delay_ms);
			 pull_down_flag = true;
		...
		{
			printk("synaptics %s: i2c communicate check success\n", __func__);
			return retval;
		}
	}
	return retval;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "failed to create synaptics_pm_wq");
	goto free_queue;
	}
...
#endif
	rmi4_data_truly = rmi4_data;
    is_tp_insert = true;

	printk("End synaptics_rmi4_probe\n");

	return retval;

free_queue:
	kfree(rmi4_data->synaptics_pm_wq);
err_sysfs:
...
err_set_input_dev:
	...
	if (bdata->power_gpio >= 0)
		synaptics_rmi4_gpio_setup(bdata->power_gpio, false, 0, 0);
	#ifdef PINCTRL
	if (rmi4_data->synaptics_pinctrl) {
		retval = pinctrl_select_state(rmi4_data->synaptics_pinctrl, rmi4_data->gpio_state_suspend);
		if (retval < 0)
		...
		bool enable)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			reporting_control,
			sizeof(reporting_control));
	printk("synaptics gesture after write:%d\n",reporting_control[2]);

	return;
}
void synaptics_rmi4_wakeup_gesture(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	/*
	if (rmi4_data->f11_wakeup_gesture)
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	printk("Begin synaptics_rmi4_suspend\n");
	if (rmi4_data->stay_awake)
	{
		printk("Already in suspend state(stay_awake)\n");
		return 0;
	}
	if (!rmi4_data->issuspended)
	{
		printk("Already in suspend state(issuspended)\n");
		return 0;
	}
	if (rmi4_data->enable_wakeup_gesture) {
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
...
exit:
	...
	mutex_unlock(&exp_data.mutex);
	printk("End synaptics_rmi4_suspend\n");
	rmi4_data->suspend = true;
	rmi4_data->issuspended = false;

	//INT & RST do not pull down,so do not use it
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	//rmi4_data_truly = rmi4_data;
	printk("Enter synaptics_rmi4_resume\n");
	if (rmi4_data->stay_awake){
		printk("synaptics already in resume state(stay_awake)!\n");
		return 0;
	}
	if(tp_sleep_mode)
	...
	if (rmi4_data->issuspended){
		printk("synaptics already in resume state(issuspended)!\n");
		return 0;
	}
continue_resume:
	...
	*/
	...
	#if 0
	msleep(100);
	SYNAP_INFO("Begin synaptics_rmi4_sw_reset");
	synaptics_rmi4_sw_reset(rmi4_data);
	SYNAP_INFO("End synaptics_rmi4_sw_reset");
	#endif
	synaptics_rmi4_irq_enable(rmi4_data, true, false);
...
exit:
	...
	rmi4_data->suspend = false;
	rmi4_data->issuspended = true;
	tp_sleep_mode = false;
	printk("End synaptics_rmi4_resume\n");
